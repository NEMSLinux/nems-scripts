    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        resolve([
          started,
          0, // dnsLookup (unused)
          0, // tcpHandshake (unused)
          0, // sslHandshake (unused)
          ttfb,
          ended,
          serverDuration,
        ]);
...
async function measureDownload(bytes, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    ...
    // Retry loop with exponential backoff to handle WAF 429s gracefully
    while (!success && attempts < 3) {
      try {
        ...
        if (error.message.includes('429') || error.message.includes('403')) {
          await sleep(1000 * attempts); // Sleep 1s, 2s, 3s depending on attempt
        } else {
          break; // Break completely if it's a non-rate-limiting error (e.g., connection lost)
        }
      }
    }
    await sleep(300); // Standard pause between successful chunks
  }
...
function logSpeedTestResult(size, test) {
  // Guarantee a valid numeric output string for programmatic parsers
