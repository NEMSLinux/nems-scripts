...
function request(options, data = '') {
  ...
  // Ensure headers exist and add a User-Agent to prevent 403s
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      // Reject if Cloudflare blocks the request so we don't calculate fake speeds
      if (res.statusCode >= 400) {
        req.destroy();
        return reject(new Error(`Server returned status ${res.statusCode}`));
      }

      // Use 'data' instead of 'readable' to reliably capture TTFB
      res.once('data', () => {
      ...
      res.on('end', () => {
        ...
        // Safely parse the server-timing header using regex
...
function measureSpeed(bytes, duration) {
  // Prevent division by zero if transfer time is inexplicably fast
...
async function measureLatency() {
  ...
  // Handle empty measurements array if requests fail
